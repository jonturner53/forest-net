...
void* handler(void *qp) {
	...
	while (true) {
		...
		if (px < 0) {
			...
			// for a remote client
			std::cout << "[DEBUG] -- px < 0 " << px << std::endl;
			int sock = -px;
			success = handleConsole(sock, cph);
            std::cout << "[DEBUG] -- outside of handleConsole " << success << std::endl;
		} else {
...
bool handleConsole(int sock, CpHandler& cph) {
    std::cout << "[DEBUG] -- inside of handleConsole" << std::endl;
