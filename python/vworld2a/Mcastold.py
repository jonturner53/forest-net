		...
		for p1 in points1 :
			for p2 in points2 :
				if self.pWorld.canSee(p1,p2) :
					return True
		return False
	#add new parameter audioAdr, edit by feng
	def updateSubs(self,x, y, audioAdr) :	
		...
		if col == self.col and row == self.row :
			if audioAdr !=None :
				if self.audioAdr != audioAdr:
					tempSub = set()
					if self.audioAdr != None :
						tempSub.add(self.audioAdr)
						self.net.unsubscribe(tempSub)
						tempSub.clear()
					tempSub.add(audioAdr)
					self.net.subscribe(tempSub)
		...
		for i in range(left,right+1) :
			...
			for j in range(bot,top+1)  :
				...
				print i+j*self.numg+1
		if audioAdr != None :
			self.mySubs.add(audioAdr)
			self.audioAdr = audioAdr
