...
####		self.msg = "initial msg"
	...
	def init(self, uname, pword) :
		...
		if not self.joinComtree() :
			...
			return False
		self.mcg.updateSubs(self.x,self.y) # for now, we don't care Z
...
# getPhoto communicates with the photo server and get the picture specified by uname
	...
	def getCallRequest(self):
		if self.pWorld.getCallRequest != 0:
			#we have a request
			#send
			self.pWorld.getCallRequest()
	...
	def sendCallRequest(self, id):
		...
		p = Packet()
		msglen = len(self.msg)
		p.leng = OVERHEAD + 4*6 + msglen; p.type = CLIENT_DATA; p.flags = 0
...
####					msglen, self.msg, self.rms, self.myFadr)
	...
	# 							MESSAGE, msglen, self.msg, namelen, self.name)
...
####								MESSAGE, msglen, self.msg, namelen, self.name)	
	...
	def updateStatus(self) :
		...
		"""
		self.x, self.y, self.z, self.direction = self.pWorld.getPosHeading()
		self.mcg.updateSubs(self.x,self.y)

####		self.msg = self.pWorld.getMsg()
####		if self.msg != '':
####			self.sendMsg(self.msg)
####			self.msg = ''

		# self.callRequest = self.pWorld.getCallRequest()
...
#			print "Got STATUS_REPORT: (x,y,z) = (%r, %r, %r)" % (x1, y1, z1)
			...
			if avId in self.nearRemotes :
				...
				remote[3] = dir1
				remote[7] = 0
				self.pWorld.updateRemote(x1,y1,z1, dir1, avId, name)
		...
		"""
		elif tuple[0] == MESSAGE:
			tuple = struct.unpack('!I', payload[4:8])
			namelen = tuple[0]
			if namelen != 0:
				name = struct.unpack(str(namelen) + 's', playload[8: 8+namelen])[0]

			tuple = struct.unpack('!I', payload[8+namelen: 12+namelen])
			msglen = tuple[0]
			if msglen != 0:
				msg = struct.unpack(str(msglen) + 's', payload[12+namelen: 12+namelen+msglen])[0]
				self.pWorld.displayMsg(msg, name)
		"""
