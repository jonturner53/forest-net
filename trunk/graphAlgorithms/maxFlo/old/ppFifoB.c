...
// Find maximum flow in G using the fifo varaint of the preflow-push algorithm.
	...
	for (u = 1; u <= G.n; u++) excess[u] = 0;

        for (e = G.first(1); e != Null; e = G.next(1,e)) {
                v = G.head(e);
		if (v == 1) continue;
                G.addflow(1,e,G.cap(1,e));
                if (v != G.n) excess[v] = G.cap(1,e);
        }

	nextedge[1] = G.first(1); nextedge[G.n] = G.first(G.n);
	while (1) {
		initdist(G,d);
		for (u = 2; u < G.n; u++) {
			nextedge[u] = G.first(u);
			if (excess[u] > 0) queue &= u;
		}
		if (queue(1) == Null) break;
	        while (queue(1) != Null) {
	                u = queue(1); queue <<= 1;
	                e = nextedge[u];
	                while (e != Null && excess[u] > 0) {
				v = G.mate(u,e);
				if (G.res(u,e) > 0 && 
				    d[u] == d[v]+1 &&
				    nextedge[v] != Null) {
