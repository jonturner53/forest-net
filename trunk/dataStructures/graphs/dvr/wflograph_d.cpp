// usage: wflograph_d
//
// Wflograph_d is a test program for the weighted flograph data structure.
//
...
//	cost v e	print cost from v on e
//	addflow v e f	add f units of flow from v on e
//	join u v w c	join two vertices with edge of given capacity and cost
...
main() {
	vertex u,v; edge e; flow c1,f1; cost c2; string cmd; wflograph G;
	...
	while (cin >> cmd) {
		if (misc::prefix(cmd,"first")) {
			if (misc::getAlpha(cin,u)) {
			    ...
			    misc::getNum(cin,c1) && misc::getNum(cin,c2)) {
                                e = G.join(u,v); G.changeCap(e,c1);
				G.changeCost(e,c2); cout << G;
                        }
