...
// Nte1 is the number of routing table entries.
	...
	for (int i = 0; i <= nte; i++) {
		tbl[i].vn = 0;  // 0 vnet designates unused entry
	}
	maxInUse = 0;
};
...
// of the table entry, or 0 if there is no match.
	for (int i = 1; i <= maxInUse; i++) {
		if (tbl[i].vn == vnet && tbl[i].adr == adr)
			return i;
	}
	return 0;
}
...
// Return true on success, false on failure.
	for (int i = 1; i <= nte; i++) {
		if (!valid(i)) {
			tbl[i].vn = vnet; tbl[i].adr = adr; tbl[i].lnk = lnk;
			maxInUse = max(i,maxInUse);
			return true;
		}
	}
	return false;
