...
private:
	...
	int	bnum(buffer_t*) const;	// integer buffer number
	buffer_t* bpntr(int) const;	// pointer to buffer with given number
...
// Return integer buffer number corresponding to b
inline int pktStore::bnum(buffer_t* b) const {
	assert(&buff[1] <= b && b <= &buff[M]);
	return b-buff;
}
// Return pointer to buffer with given index
inline buffer_t* pktStore::bpntr(int bx) const {
	assert(1 <= bx && bx <= N);
	return &buff[bx];
}
