// Header file for lnkTbl class, which stores information
// about all the links incident to a given router.
//

#ifndef LNKTBL_H
#define LNKTBL_H

#include "lfs.h"
#include "hashTbl.h"

class lnkTbl {
public:
		lnkTbl(int=31);
		~lnkTbl();
	int	lookup(int,ipa_t,ipp_t,ipa_t);	// retrieve matching entry
	bool	addEntry(int,int,ipa_t,ipa_t,ntyp_t); // create new entry
	bool	removeEntry(int);		// remove entry from table
	bool	checkEntry(int);		// perform consistency check
	bool	valid(int) const;		// return true for valid entry
	void	enable(int);			// make table entry valid
	void	disable(int);			// make table entry invalid
	// routines for accessing various fields
	int	interface(int) const;	
	ipa_t	peerIpAdr(int) const;	
	ipp_t	peerPort(int) const;	
	bool 	setPeerPort(int,ipp_t);		// set peer port
	ntyp_t  peerTyp(int) const;	
	ipa_t	peerAdr(int) const;		// address in overlay	
	int&	bitRate(int);	
	int&	pktRate(int);	
	int	minDelta(int) const;	
	// statistics routines
	uint32_t iPktCnt(int) const;
	uint32_t oPktCnt(int) const;
	uint32_t iBytCnt(int) const;
	uint32_t oBytCnt(int) const;
	void	postIcnt(int, int);
	void	postOcnt(int, int);
	// io routines
	int	getEntry(istream&);	 	// read one new table entry
	friend	bool operator>>(istream&, lnkTbl&); // read entire table
	void	putEntry(ostream&, int); 	// write a single entry
	friend	ostream& operator<<(ostream&, lnkTbl&); // print table
private:
	int	nlnk;			// max number of links in table

	struct lnkdata {
	int	intf;			// interface number for link
	ipa_t	pipa;			// IP address of peer endpoint
	ipp_t	pipp;			// peer port number
	ntyp_t	ptyp;			// node type of peer
	ipa_t	padr;			// peer's forest address
	int	bitrate;		// maximum bit rate of link (MAC level)
	int	pktrate;		// maximum packet rate of link
	uint32_t iPkt;			// input packet counter
	uint32_t oPkt;			// output packet counter
	uint32_t iByt;			// input byte counter
	uint32_t oByt;			// output byte counter
	};
	lnkdata	*ld;			// ld[i] is link data for link i
	hashTbl *ht;			// hash table for fast lookup
	uint64_t hashkey(ipa_t,ipa_t) const;
					// compute key for hash lookup
};

inline bool lnkTbl::valid(int i) const { return (ld[i].padr != 0); }
inline void lnkTbl::enable(int i) { if (ld[i].padr == 0) ld[i].padr++; }
inline void lnkTbl::disable(int i) { ld[i].padr = 0; }

// Routines to access various fields in lnkTbl entries
inline int lnkTbl::interface(int i) const	{ return ld[i].intf; }
inline ipa_t lnkTbl::peerIpAdr(int i) const	{ return ld[i].pipa; }
inline ipp_t lnkTbl::peerPort(int i) const	{ return ld[i].pipp; }
inline ntyp_t lnkTbl::peerTyp(int i) const	{ return ld[i].ptyp; }
inline ipa_t lnkTbl::peerAdr(int i) const	{ return ld[i].padr; }
inline int& lnkTbl::bitRate(int i) 		{ return ld[i].bitrate; }
inline int& lnkTbl::pktRate(int i) 		{ return ld[i].pktrate; }
inline int lnkTbl::minDelta(int i) const {
	return int(1000000.0/ld[i].pktrate);
}

// Set peer's port number, but only if peer is not a router and
// new port number is not the one reserved for routers.
inline bool lnkTbl::setPeerPort(int te,ipp_t pipp) {
	if (peerTyp(te) != ROUTER && pipp != LFS_PORT) {
		ld[te].pipp = pipp; return true;
	}
	return false;
}

// statistics routines
inline uint32_t lnkTbl::iPktCnt(int i) const { return ld[i].iPkt; }
inline uint32_t lnkTbl::oPktCnt(int i) const { return ld[i].oPkt; }
inline uint32_t lnkTbl::iBytCnt(int i) const { return ld[i].iByt; }
inline uint32_t lnkTbl::oBytCnt(int i) const { return ld[i].oByt; }
inline void lnkTbl::postIcnt(int i, int leng) {
	ld[i].iPkt++; ld[i].iByt += truPktLeng(leng);
}
inline void lnkTbl::postOcnt(int i, int leng) {
	ld[i].oPkt++; ld[i].oByt += truPktLeng(leng);
}

// Compute key for hash lookup
inline uint64_t lnkTbl::hashkey(ipa_t adr1, ipa_t adr2) const {
	return (uint64_t(adr1) << 32) | uint32_t(adr2);
}

// Return index of link for a packet received on specified interface,
// with specified source IP (address,port) pair and the given
// source LFS address. If no match, return Null.
inline int lnkTbl::lookup(int intf, ipa_t pipa, ipp_t pipp, ipa_t slfs) {
	ipa_t x = (pipp != LFS_PORT ? slfs : pipa);
	int te = ht->lookup(hashkey(pipa,x));
	if (te !=0 && intf == interface(te) && 
	    (pipp == peerPort(te) || peerPort(te) == 0))
		return te;
	return 0;
}

#endif
