...
class Avatar(DirectObject):
	def __init__(self, userName, avaModel):
		...
		self.environ.setPos(0,0,0)
 		...
 		base.setBackgroundColor(0.25,0.3,0.5,1)
		...
		elif self.avaModel == 2 :
			self.avatar.setScale(.002)
			self.avatar.setPos(58,67,0)
		elif self.avaModel == 3 :
			self.avatar.setScale(.002)
			self.avatar.setPos(58,67,.5)
		elif self.avaModel == 4 :
			self.avatar.setScale(.002)
			self.avatar.setPos(58,67,.5)
		...
		self.avPos = printText(0.9)
 		...
 		# Show a list of visible avatars
...
#		self.accept("control-s", self.switchView)
		...
		# Show a list of instructions for zooming
		# Maybe should have a help key for showing complete
		# set of instructions
	...
	def getPosHeading(self) :
		...
		return (self.avatar.getX(), self.avatar.getY(), \
			self.avatar.getZ(), (self.avatar.getHpr()[0])%360)
	...
	def move(self, task):
		...
		# fov: field of view, 20--80 is acceptable range, others distort
		...
		""" not relvant since we use the first-person viewpoint
		...
		for id in self.remoteMap:
			self.remoteMap[id][3].setPos( \
				(self.remoteMap[id][0].getX() / \
					self.getLimit()+0.0)*0.7+0.45, \
				0, (self.remoteMap[id][0].getY() / \
					self.getLimit()+0.0)*0.7+0.25)
