...
public class ConnectionComtCtl {
	...
	protected String getNet() {
		...
		try {
			inRdr = new PushbackReader(new InputStreamReader(serverChan
	...
	/**
	 * Read a response to a getComtSet request. The response consists of the
	 * string "comtSet" followed by a list of comtree numbers, separated by
	...
	private String readComtSet(PushbackReader in) {
		...
		while (true) {
			if (!Util.readNum(in, cnum))
				return "readComtSet: could not read "
