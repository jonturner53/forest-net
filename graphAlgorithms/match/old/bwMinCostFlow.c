// usage: bwMinCostFlow
//
// bwMinCostFlow reads a weighted bipartite graph from stdin, computes a
// maximum weight matching using the min cost flow method and
// prints it out.
//
...
main() {
		...
		e = M(1);
                if (G.n <= 26)
                        printf("(%c,%c,%2d) ",
                                ('a'-1)+G.left(e),('a'-1)+G.right(e),G.w(e));
                else
                        printf("(%2d,%2d,%3d) ",G.left(e),G.right(e),G.w(e));
                if ((i++)%5 == 4) printf("\n");
                M <<= 1;
        }
        printf("\n");
