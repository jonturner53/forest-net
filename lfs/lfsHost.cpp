// usage:
//      lfsHost myIpAdr rtrIpAdr lfsAdr repeatFlag delta finTime
//
// LfsHost is a simple packet generator for an lfs host.
// MyIpAdr and rtrIpAdr are the IP addresses of the host itself
// and the forest router it connects to. MyAdr is the lfs adress
// of the host (that is, its IP address in the lfs overlay). Delta
// is the minimum number of microseconds between successive packet
// transmissions. FinTime is the number of seconds that the host
// should run.
//
// The packets generated by lfsHost are determined by a packet
...
//
// If repeatFlag=0, lfsHost sends each of the listed packets one time,
// ignoring all repeat specifications.
//
// LfsHost also receives incoming packets from its attached router.
...
main(int argc, char *argv[]) {
	...
	ipa_t myIpAdr, rtrIpAdr; ipa_t lfsAdr, lfsa0, lfsa1, lfsa2, lfsa3;
	int repeatFlag, delta, finTime;

	if (argc != 7 ||
	    sscanf(argv[1],"%d.%d.%d.%d", &mip3, &mip2, &mip1, &mip0) != 4 ||
	    sscanf(argv[2],"%d.%d.%d.%d", &rip3, &rip2, &rip1, &rip0) != 4 ||
	    sscanf(argv[3],"%d.%d.%d.%d", &lfsa3, &lfsa2, &lfsa1, &lfsa0) != 4 ||
	    sscanf(argv[4],"%d", &repeatFlag) != 1 ||
	    sscanf(argv[5],"%d", &delta) != 1 ||
	    sscanf(argv[6],"%d", &finTime) != 1)
		fatal("usage: lfsHost myIpAdr routerIpAdr myLfsAdr repeatCnt delta finTime");

	 myIpAdr = ((mip3 & 0xff) << 24)  | ((mip2 & 0xff) << 16) |
	           ((mip1 & 0xff) <<  8)  |  (mip0 & 0xff);
	rtrIpAdr = ((rip3 & 0xff) << 24)  | ((rip2 & 0xff) << 16) |
	           ((rip1 & 0xff) <<  8)  |  (rip0 & 0xff);
	  lfsAdr = ((lfsa3 & 0xff) << 24)  | ((lfsa2 & 0xff) << 16) |
	           ((lfsa1 & 0xff) <<  8)  |  (lfsa0 & 0xff);

	lfsHost host(myIpAdr,rtrIpAdr,lfsAdr);
	if (!host.init()) fatal("lfsHost:: initialization failure");
	host.run((repeatFlag ? true : false), delta, 1000000*finTime);
}


// Constructor for lfsHost, allocates space and initializes private data
lfsHost::lfsHost(ipa_t mipa, ipa_t ripa, ipa_t lfsa)
	      : myIpAdr(mipa) , rtrIpAdr(ripa), myLfsAdr(lfsa) {
	nPkts = 10000;
...
bool lfsHost::getPacket(packet p, int& pause, int& cnt) {
...
// Return next waiting packet or Null if there is none. 
	...
	if (nbytes < 0) {
		...
		fatal("lfsHost::receive: error in recvfrom call");
	}

	h.ioBytes() = nbytes;
 	h.srcIp() = ntohl(ssa.sin_addr.s_addr);
        h.srcPort() = ntohs(ssa.sin_port);

	return p;
}

void lfsHost::send(packet p) {
// Send packet on specified link and recycle storage.
	dsa.sin_addr.s_addr = htonl(rtrIpAdr);
	dsa.sin_port = htons(LFS_PORT);
	header& h = ps->hdr(p);
	int rv = sendto(sock,(void *) &(ps->buffer(p))[0],h.leng(),0,
		    	(struct sockaddr *) &dsa, sizeof(dsa));
	if (rv == -1) fatal("lfsHost::send: failure in sendto");
}

void lfsHost::run(bool repeatFlag, uint32_t delta, uint32_t finishTime) {
...
// FinishTime and delta are expressed in microseconds.
	...
	int i, pause, cnt, np; packet p;
	const int MAXEVENTS = 200;
	struct { bool sendFlag; uint32_t time; packet pkt;} events[MAXEVENTS];
	int evCnt = 0;
	...
	bool didNothing;
	struct { int pause, cnt, iter; packet p; } pkt[nPkts+1];

	// read packets from file into packetStore
	np = 0;
	while (np < nPkts) {
		p = ps->alloc();
		if (p == 0) fatal("lfsHost::run: too many packets");
	...
	while (now <= finishTime) {
		...
		pt = ct;
		if (gettimeofday(&ct,Null) < 0)
			fatal("lfsHost::run: gettimeofday failure");
