...
public class ComtreeDisplay{
	...
	/**
		...
		if (repBuf == null) {
                        repBuf = ByteBuffer.allocate(10*size);
                        repBuf.clear();
                }
                ...
                if (needData) {
					 ...
					 if (comChan.read(repBuf) == 0)
                                                return null; 
					System.out.println("read from comChan, position=" + repBuf.position() + " " + System.currentTimeMillis());
	...
	public static void main(String[] args){
		...
		while(true){
			...
			try {
				if(pkt != null)
					out.write(pkt[0] + " " + pkt[1] + " " + pkt[2] + " " + pkt[3] + "\n");
					out.flush();
					...
					for(Pair p: routers.keySet()){
						...
						if(pkt!=null){
							...
							if(pkt[0] == Integer.parseInt(comtree) && pkt[1]==Integer.parseInt(r.getZip())) {
								...
								//System.out.println("in: " + pkt[0] + " " + pkt[1] + " " + pkt[2] + "\n");
							}
						}
